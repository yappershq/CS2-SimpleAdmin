...
public class Migration(Database database)
{
    public void ExecuteMigrations()
    {
        try
        {
            var migrationsDirectory =
                CS2_SimpleAdmin.Instance.ModuleDirectory + "/Database/Migrations";

            var files = Directory.GetFiles(migrationsDirectory, "*.sql").OrderBy(f => f);
            ...
            foreach (var file in files)
            {
                ...
                if (
                    string.Compare(version, lastAppliedVersion, StringComparison.OrdinalIgnoreCase)
                    <= 0
                )
                    continue;
    ...
    {
        using var cmd = new MySqlCommand(
            "SELECT `version` FROM `sa_migrations` ORDER BY `id` DESC LIMIT 1;",
            connection
        );
    ...
    {
        using var cmd = new MySqlCommand(
            "INSERT INTO `sa_migrations` (`version`) VALUES (@Version);",
            connection
        );
